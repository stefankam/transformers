...
from transformers.models.t5_dynamic.configuration_t5_dynamic import T5DynamicConfig


logger = logging.get_logger(__name__)

_CONFIG_FOR_DOC = "T5DynamicConfig"
...
class T5DenseActDense(nn.Module):
    def __init__(self, config: T5DynamicConfig):
        super().__init__()
...
class T5LayerFF(nn.Module):
    ...
    def forward(self, hidden_states, updated_hidden_states=None):
        """
        If updated_hidden_states is provided, concatenate it with the original hidden_states.
        This simulates the dynamic memory refresh (from the encoder).
        """
        if updated_hidden_states is not None:
        # Update hidden states by concatenating past and new hidden states
            hidden_states = torch.cat([hidden_states, updated_hidden_states], dim=-1)  # Concatenating along the feature dimension
    ...
    def forward(
        ...
        if position_bias is None:
            ...
            if mask is not None:
                ...
                # Ensure position_bias shape matches causal_mask
                if position_bias.shape[-1] != causal_mask.shape[-1]:  # Dimension mismatch
                     expected_size = causal_mask.shape[-1]
                     position_bias = position_bias[..., :expected_size]  # Trim to match causal_mask
                position_bias = position_bias + causal_mask

        if self.pruned_heads:
            mask = torch.ones(position_bias.shape[1], dtype=torch.bool, device=position_bias.device)
            mask[list(self.pruned_heads)] = False
            position_bias_masked = position_bias[:, mask]
        else:
            position_bias_masked = position_bias

            # Ensure final shape matches `scores`
            # Pad position_bias_masked to match the scores shape if it is smaller
        if position_bias_masked.shape[-1] < scores.shape[-1]:
            pad_size = scores.shape[-1] - position_bias_masked.shape[-1]
            pad_tensor = torch.zeros(position_bias_masked.shape[:-1] + (pad_size,),
            device=position_bias_masked.device, dtype=position_bias_masked.dtype)
            position_bias_masked = torch.cat([position_bias_masked, pad_tensor], dim=-1)
            #print(f"Padded position_bias_masked to shape: {position_bias_masked.shape}")
    
        # If it was larger, truncate
        elif position_bias_masked.shape[-1] > scores.shape[-1]:
            position_bias_masked = position_bias_masked[..., :scores.shape[-1]]
            #print(f"Truncated position_bias_masked to shape: {position_bias_masked.shape}")
...
T5_START_DOCSTRING = r"""
    ...
    Parameters:
        config ([`T5DynamicConfig`]): Model configuration class with all the parameters of the model.
...
class T5Model(T5PreTrainedModel):
    ...
    def forward(
        ...
        if encoder_input_ids is not None:
            ...
            # Ensure the decoder incorporates updated attention
            if self.decoder.attention.self.has_relative_attention_bias:
                # If using relative attention bias, we need to recompute position bias
                self.decoder.attention.self.position_bias = None  # Force recalculation
...
class T5ForConditionalGeneration(T5PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: T5DynamicConfig):
        super().__init__(config)
        super().__init__(config)
        # Initialization of encoder, decoder, etc. (unchanged)
